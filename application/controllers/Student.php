...
class Student extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array("timeline_model", "student_edit_field_model", 'transportfee_model', 'marksdivision_model', 'module_model','location_model'));
    ...
    public function create($enquiry_id=null)
    {
        ...
        $locationsList            = $this->location_model->getlocationlist();
        $data['houses']       = $houses;
        ...
        if ($this->form_validation->run() == false) {
			...
			if(!is_null($enquiry_id)){
				$data['enquiry_details'] = $this->enquiry_model->getenquiry_list($enquiry_id, $status = 'active');
				//print_r($data['enquiry_details']);die;
			}
				$this->load->view('layout/header', $data);
            ...
            if (isset($location)) {
                $data_insert['location_id'] = $this->input->post('location');
            }
    ...
    public function download_form()
    {
        ...
        $this->load->view('student/download_student_form' ,array('data'=>$setting));
    }
    ...
    public function edit($id)
    {
        ...
        $data['categorylist']       = $category;
        $hostelList                 = $this->hostel_model->get();
        $locationList            = $this->location_model->getlocationlist();
    ...
    public function profilesetting()
    {
        ...
        $this->load->view("layout/setting_header");
    ...
    public function dtstudentlist()
    {
        ...
        if (!empty($students->data)) {
            foreach ($students->data as $student_key => $student) {
				...
				$viewbtn = "<a target='_blank' href='" . base_url() . "student/view/" . $student->id . "' class='btn btn-icon btn-sm btn-white btn-dim btn-outline-info p-1 me-1' data-toggle='tooltip' aria-label='" . $this->lang->line('view') . "' data-bs-original-title='". $this->lang->line('view') ."'><em class='ni ni-eye'></em></a>";

                if ($this->rbac->hasPrivilege('student', 'can_edit')) {
                    $editbtn = "<a target='_blank' href='" . base_url() . "student/edit/" . $student->id . "'   class='btn btn-icon btn-sm btn-white btn-dim btn-outline-info p-1 me-1'  data-toggle='tooltip' aria-label='" . $this->lang->line('edit') . "' data-bs-original-title='" . $this->lang->line('edit') . "'><em class='ni ni-edit'></em></a>";
                }
                if ($this->module_lib->hasActive('fees_collection') && $this->rbac->hasPrivilege('collect_fees', 'can_add')) {

                    $collectbtn = "<a target='_blank' href='" . base_url() . "studentfee/addfee/" . $student->student_session_id . "'   class='btn btn-icon btn-sm btn-white btn-dim btn-outline-info p-1'  data-toggle='tooltip' aria-label='" . $this->lang->line('add_fees') . "' data-bs-original-title='" . $this->lang->line('add_fees') . "'><em class='ni ni-sign-inr-alt'></em></a>";
                }

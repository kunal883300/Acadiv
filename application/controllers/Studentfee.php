...
class Studentfee extends Admin_Controller
{
    ...
    public function feesearch()
    {
        ...
        print_r($currentsessiontransportfee);
...
// print_r($data);
        if ($this->form_validation->run() == false) {
            ...
            if (!empty($student_due_fee)) {
                foreach ($student_due_fee as $student_due_fee_key => $student_due_fee_value) {
                    ...
                    // Add fee details regardless of payment status
                    $students[$student_due_fee_value['student_session_id']]['fees'][] = array(
                        ...
                        'status'          => ($amt_due <= ($amount + $amount_discount)) ? 'Paid' : 'Due'
                    );
                }
            }
    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_flashdata('msg', '<div studentfee="alert alert-success text-center">' . $this->lang->line('success_message') . '<button class="close" data-bs-dismiss="alert"></button></div>');
            redirect('studentfee/index');
        }
    }
    ...
    public function bulk_due_fees()
    {
        ...
        // Retrieve the paid_data array
        $paid_data = $this->input->post('paid_data');
        
        // Decode the JSON strings back to objects
        $decoded_paid_data = array_map(function($data) {
            return json_decode($data);
        }, $paid_data);
        // Calculate the sum of the amounts in the paid_data array
        $sum_paid_data_amount = array_sum(array_column($decoded_paid_data, 'amount'));
        
        // Check if the sum of paid data amounts is less than the amount
        if ($sum_paid_data_amount < $amount) {
            $array = array('status' => 'fail', 'error' => "Insufficient data");
            echo json_encode($array);
            return;
        }
    
        // Process each paid data entry and insert into the database
        foreach ($decoded_paid_data as $data) {
            $fee_amount =0;
            $total_amount =0;
            $student_id = $data->student_session_id;
            ...
            if ($data->amount_detail != 0) {
                // Decode the JSON amount_detail
                $amount_detail = json_decode($data->amount_detail, true);
                ...
                // $total_amount = array_sum(array_column($amount_detail, 'amount'));
                $fee_amount -= $total_amount; // Calculate remaining fee amount
               
                
                // Get the last element and its index
                $last_element = end($amount_detail);
                $last_index = key($amount_detail) +1;
                $last_inv_no = $last_element['inv_no'] +1;
            
                
            }
            
            // If there is still some amount to be settled
            if ($amount > 0 && $fee_amount > 0) {
                // Calculate the amount to be deducted for the current fee entry
                $deducted_amount = min($amount, $fee_amount);
               
                // Prepare the JSON array for the fee deposit
                $json_array = array(
                    'amount'          => $deducted_amount,
                    'date'            => date('Y-m-d'), // Current date
                    'description'     => "pending fees paid",
                    'amount_discount' => "0",
                    'collected_by'    => $collected_by,
                    'amount_fine'     => "0",
                    'payment_mode'    => "cash",
                   'received_by'     => $staff_record['id'],
                    'inv_no'          => $data->amount_detail != 0 ? $last_inv_no :"1"
                );
    
                if($data->amount_detail != 0){
                    $partial =1;
                     // Insert the data into the database
                $this->studentfeemaster_model->insert_fee_deposit(array(
                    'partial' => 1,
                    'student_fees_master_id' => $data->id,
                    'fee_groups_feetype_id'  => $data->fee_groups_feetype_id,
                    'amount_detail'          =>  json_encode(array($last_index => $json_array)),
                    'is_active'              => 1,
                    ...
                    $partial =0;
                     // Insert the data into the database
                $this->studentfeemaster_model->insert_fee_deposit(
                    array(
                   
                    'student_fees_master_id' => $data->id,
                    'fee_groups_feetype_id'  => $data->fee_groups_feetype_id,
                    'amount_detail'          => json_encode(array('1' => $json_array)),
                    'is_active'              => 1,
                    'created_at'             => date('Y-m-d H:i:s')
                ), $partial);
                }
                ...
                // Subtract the processed amount from the total amount
                $amount -= $deducted_amount;
        ...
        redirect('studentfee/addfee/'.$student_id);
    }

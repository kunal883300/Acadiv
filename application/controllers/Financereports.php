...
class Financereports extends Admin_Controller
{
    ...
    public function studentacademicreport()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($class_id)) {
                $studentlist = $this->student_model->locationsearchByClassSectionWithSession($class_id, $section_id,$location_id);
            } else {
                $studentlist = $this->student_model->getLocationStudents();
            }
            $student_Array = array();
            if (!empty($studentlist)) {
                foreach ($studentlist as $key => $eachstudent) {
                    ...
                    $obj->location_name   = $eachstudent['location_name'];
    ...
    public function collection_report()
    {
        ...
        if (isset($_POST['fee_groups_id']) && $_POST['fee_groups_id'] != '') {
            $fee_groups_id = $_POST['fee_groups_id'];
        } else {
            $fee_groups_id = "";
        }
        ...
        $data['results'] = $this->studentfeemaster_model->getFeeCollectionReport($start_date, $end_date, $feetype_id, $received_by, $group, $class_id, $section_id,$fee_groups_id);
    ...
    public function feesearch()
    {
        ...
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'Reports/finance');
        $this->session->set_userdata('subsub_menu', 'Reports/finance/fee_report_by_group');
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view('layout/header', $data);
            $this->load->view('financereports/fee_report_by_group', $data);
            $this->load->view('layout/footer', $data);
        } else {

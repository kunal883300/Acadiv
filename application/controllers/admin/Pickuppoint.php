        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                ...
                if ($this->rbac->hasPrivilege('pickup_point', 'can_edit')) {                    
                    $action = '<button class="btn btn-icon btn-sm btn-white btn-dim btn-outline-success p-1 pickup_map" data-pick-location="' . $value->id . '"data-bs-toggle="tooltip" title="' . $this->lang->line("map") . '"><i class="fa fa-map-marker"></i></button><a onclick="edit(' . $value->id . ')" class="btn btn-icon btn-sm btn-white btn-dim btn-outline-primary p-1"  data-bs-toggle="modal" data-bs-target="#myModalEdit" title="' . $this->lang->line('edit') . '"><em class="ni ni-edit"></em></a>';
                }else{
                    $action = '';
                }
                
                if ($this->rbac->hasPrivilege('pickup_point', 'can_delete')) {
                    $deletebtn = " <a href=" . base_url() . 'admin/pickuppoint/delete_point/' . $value->id . " class='btn btn-icon btn-sm btn-white btn-dim btn-outline-danger p-1' data-bs-toggle='tooltip'  title=" . $this->lang->line('delete') . " '  onclick='return confirm(" . '"' . $this->lang->line('delete_confirm') . '"' . "  )' ><em class='ni ni-cross'></em></a>";
                }else{
                    $deletebtn = '';
                }

...
class Admin extends Admin_Controller
{
    ...
    public function dashboard()
    {
        ...
        $data['student_due_fee'] = $this->studentacademicreport();
        $totalfee = 0;
        $totaldeposit = 0;
        $discountlabel = 0;
        $finelabel = 0;
        $balancelabel = 0;  
        foreach($data['student_due_fee'] as $student_detail){
            $totalfee += $student_detail->totalfee;
            $totaldeposit += $student_detail->deposit;
            $discountlabel += $student_detail->discount;
            $finelabel += $student_detail->fine;
            $balancelabel += $student_detail->balance;
        }
        
        $data['totalfee'] = $totalfee;
        $data['totaldeposit'] = $totaldeposit;
        $data['discountlabel'] = $discountlabel;
        $data['finelabel'] = $finelabel;
        ...
        if (!empty($studentlist)) {
            foreach ($studentlist as $eachstudent) {
                ...
                if (!empty($student_total_fees)) {
                    foreach ($student_total_fees as $fees) {
                        if (!empty($fees->fees)) {
                            foreach ($fees->fees as $fee) {
                                $totalfee += $fee->amount;
    
                                if (isJSON($fee->amount_detail)) {
    ...
    public function manageDayBook()
    {
        ...
        // Call the search method to get income data
        $incomeList = $this->income_model->incomeDayBook("", $start_date, $end_date);
        ...
        if (!empty($incomeList)) {
            foreach ($incomeList as $value) {
                $income_grand_total += $value['amount'];
        ...
        $data['income_grand_total'] = $income_grand_total;
        // Call the search method to get expense data
        $expenseList = $this->expense_model->expenseDayBook("", $start_date, $end_date);
    $currency_symbol = $this->customlib->getSchoolCurrencyFormat();
    $dt_data = array();
    $expense_grand_total = 0;
    if (!empty($expenseList)) {
        foreach ($expenseList as $value) {
            $expense_grand_total += $value['amount'];
            $row = array();
            $row[] = $value['name'];
            $row[] = $value['invoice_no'];
    ...
    $data['expense_grand_total'] = $expense_grand_total;
    //fees 
                // Create a DateTime object from the input date
            $x = new DateTime($dates['from_date']);
            $y = new DateTime($dates['to_date']);
            // Format the date to MM-DD-YYYY
            $xStart = $x->format('m-d-Y');
            $xEnd = $y->format('m-d-Y');
    $formated_date_from = strtotime($this->customlib->dateFormatToYYYYMMDD($xStart));
    $formated_date_to   = strtotime($this->customlib->dateFormatToYYYYMMDD($xEnd));
    ...
    foreach ($fees_data as $fee_key => $fee_value) {
     ...
     $cash = ($total_amount + $income_grand_total) - $expense_grand_total;
    ...
    public function backup()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == "POST") {
            ...
            if ($this->input->post('backup') == "backup") {
                ...
                $this->session->set_flashdata('msg', '<div class="alert alert-success text-left alert-icon alert-dismissible"><em class="icon ni ni-check-circle"></em>' . $msg . '</div>');
                redirect('admin/admin/backup');
            }
        }
        ...
        $this->load->view('layout/setting_header', $data);
    ...
    public function dtstudentlist()
    {
        ...
        if (!empty($students->data)) {
            foreach ($students->data as $student_key => $student) {
                ...
                $viewbtn    = '';
                $collectbtn = "";
                $viewbtn    = "<a href='" . base_url() . "student/view/" . $student->id . "'   class='btn btn-icon btn-sm btn-white btn-dim btn-outline-primary p-1'  data-toggle='tooltip' title='" . $this->lang->line('show') . "'><em class='ni ni-eye'></em></a>";

                if ($this->rbac->hasPrivilege('student', 'can_edit')) {
                    $editbtn = "<a href='" . base_url() . "student/edit/" . $student->id . "'   class='btn btn-icon btn-sm btn-white btn-dim btn-outline-primary p-1'  data-toggle='tooltip' title='" . $this->lang->line('edit') . "'><em class='ni ni-edit-alt'></em></a>";
                }
                if ($this->module_lib->hasActive('fees_collection') && $this->rbac->hasPrivilege('collect_fees', 'can_add')) {

                    $collectbtn = "<a href='" . base_url() . "studentfee/addfee/" . $student->student_session_id . "'   class='btn btn-icon btn-sm btn-white btn-dim btn-outline-primary p-1'  data-toggle='tooltip' title='" . $this->lang->line('add_fees') . "'><span ><em class='ni ni-sign-inr'></em></a>";
                }

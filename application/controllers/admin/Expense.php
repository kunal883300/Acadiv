    ...
    public function getexpenselist()
    {
        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                ...
                if ($this->rbac->hasPrivilege('expense', 'can_edit')) {
                    $editbtn = "<a href='" . base_url() . "admin/expense/edit/" . $value->id . "'   class='btn btn-icon btn-sm btn-white btn-dim btn-outline-primary p-1'  data-toggle='tooltip' title='" . $this->lang->line('edit') . "'><em class='ni ni-edit'></em> </a>";
                } 
                if ($this->rbac->hasPrivilege('expense', 'can_delete')) {
                    $deletebtn = '';
                    $deletebtn = "<a onclick='return confirm(" . '"' . $this->lang->line('delete_confirm') . '"' . ");' href='" . base_url() . "admin/expense/delete/" . $value->id . "' class='btn btn-icon btn-sm btn-white btn-dim btn-outline-danger p-1' title='" . $this->lang->line('delete') . "' data-toggle='tooltip'><em class='ni ni-cross'></em> </a>";
                }

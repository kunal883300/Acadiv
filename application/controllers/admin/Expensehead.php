...
class Expensehead extends Admin_Controller
{
        ...
        if (!empty($expense_head->data)) {           
            ...
            foreach ($expense_head->data as $exhead_key => $exhead_value) {
                ...
                if ($this->rbac->hasPrivilege('expense_head', 'can_edit')) {
                    $action .= "<a href='" . site_url('admin/expensehead/edit/' . $exhead_value->id) . "' class='btn btn-icon btn-sm btn-white btn-dim btn-outline-primary p-1'  data-toggle='tooltip' title='" . $this->lang->line('edit') . "'><em class='ni ni-edit'></em></a>";
                }
                if ($this->rbac->hasPrivilege('expense_head', 'can_delete')) {
                    $action .= "<a href='" . site_url('admin/expensehead/delete/' . $exhead_value->id) . "' class='btn btn-icon btn-sm btn-white btn-dim btn-outline-danger p-1'  data-toggle='tooltip' title='" . $this->lang->line('delete') . "' onclick='return confirm(" . '"' . $this->lang->line('delete_confirm') . '"' . ");'><em class='ni ni-cross'></i></a>";
                }
                $row           = array();
    ...
    public function delete($id)
    {
        ...
        $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-icon alert-dismissible"><em class="icon ni ni-check-circle"></em>  '.$this->lang->line('delete_message').'<button class="close" data-bs-dismiss="alert"></button></div>');
    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_flashdata('msg', '<div class="alert alert-success alert-icon alert-dismissible"><em class="icon ni ni-check-circle"></em>  '.$this->lang->line('success_message').'<button class="close" data-bs-dismiss="alert"></button></div>');
    ...
    public function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->expensehead_model->add($data);
            $this->session->set_flashdata('msg', '<div class="alert alert-success alert-icon alert-dismissible"><em class="icon ni ni-check-circle"></em>  '.$this->lang->line('update_message').'<button class="close" data-bs-dismiss="alert"></button></div>');

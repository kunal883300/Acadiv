...
class Examresult extends Admin_Controller
{
    ...
    public function print_marksheet(){
        $marksheet_template	=	$this->uri->segment(4);
		$exam_group_class_batch_exam_student_id				=	$this->uri->segment(5);		
		$student_id				=	$this->uri->segment(6);		
		$post_exam_id				=	$this->uri->segment(7);
		$post_exam_group_id			=	$this->uri->segment(9);
        ...
        $data['sch_setting'] = $this->sch_setting_detail;
         $this->load->view('admin/examresult/_printpdfmarksheet', $data);
    }
    ...
    $stylesheet = file_get_contents(base_url() . 'backend/pdf_style.css');
    ...
    $pdfFilePath = FCPATH . "uploads/temp/" . uniqid() . ".pdf";
    $mpdf->Output($pdfFilePath, 'F');

    $pdfUrl = base_url() . "uploads/temp/" . basename($pdfFilePath);
        ...
        if ($type == "email") {
            ...
            $array = array('status' => 1, 'pdf_url' => $pdfUrl);
    echo json_encode($array);
        } else {
            // Provide URL to preview
            $array = array('status' => 1, 'pdf_url' => $pdfUrl);

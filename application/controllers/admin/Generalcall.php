    ...
    public function getcalllist()
    {
        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                ...
                $viewbtn = "<a onclick='getRecord(" . $value->id . ")' class='btn btn-icon btn-sm btn-white btn-dim btn-outline-success p-1' data-bs-target='#calldetails' data-bs-toggle='modal'  title='" . $this->lang->line('view') . "'><i class='fa fa-reorder'></i></a>";

                if ($this->rbac->hasPrivilege('phone_call_log', 'can_edit')) {
                    $editbtn = "<a href='" . base_url() . "admin/generalcall/edit/" . $value->id . "'   class='btn btn-icon btn-sm btn-white btn-dim btn-outline-primary p-1'  data-toggle='tooltip' title='" . $this->lang->line('edit') . "'><em class='ni ni-edit'></em>                    </a>";
                }
                if ($this->rbac->hasPrivilege('phone_call_log', 'can_delete')) {
                    $deletebtn = '';
                    $deletebtn = "<a onclick='return confirm(" . '"' . $this->lang->line('delete_confirm') . '"' . "  )' href='" . base_url() . "admin/generalcall/delete/" . $value->id . "' class='btn btn-icon btn-sm btn-white btn-dim btn-outline-danger p-1' title='" . $this->lang->line('delete') . "' data-toggle='tooltip'><em class='ni ni-cross'></em>
                    </a>";
                }
                $row   = array();
                $row[] = $value->name;

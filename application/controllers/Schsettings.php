...
class Schsettings extends Admin_Controller
{
    ...
    public function generalsetting()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = array(
                ...
                'founded_year'       => $this->input->post('founded_year'),
                'dise_code'       => $this->input->post('dise_code'),
                'affiliation_year'       => $this->input->post('affiliation_year'),
    ...
    public function fees()
    {
        ...
        $setting                        = $this->setting_model->getSetting();
        $fees_due_days = $setting->fee_due_days;
        if ( $fees_due_days != "") {

            $due_date                  = date('Y-m-d', strtotime('+' . $fees_due_days . ' day'));
            $data['due_date_formated'] = date($setting->date_format, $this->customlib->dateYYYYMMDDtoStrtotime($due_date));
        } else {
    ...
    public function savefees()
    {
        ...
        $this->form_validation->set_rules('selected_date', $this->lang->line('carry_forward_fees_due_days'), 'trim|required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            echo json_encode($array);
                        ...
                        // Given date
            $givenDate = $this->input->post('selected_date'); // Format: YYYY-MM-DD

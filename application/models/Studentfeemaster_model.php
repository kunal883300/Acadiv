...
class Studentfeemaster_model extends MY_Model
{
    ...
    public function insert_fee_deposit($data,$partial)
    {
        if ($partial == 0) {
            return $this->db->insert('student_fees_deposite', $data);
        } else {
            // Retrieve the existing amount_detail from the database
            ...
            if ($query->num_rows() > 0) {
                $row = $query->row();
                $existing_amount_detail = json_decode($row->amount_detail, true);
    
                // Decode the new amount_detail from the input data
                $new_amount_detail = json_decode($data['amount_detail'], true);
    
                // Initialize a new array for merged data with sequential keys
                $merged_amount_detail = [];
                
                // Add existing data to the merged array with sequential keys
                $index = 1;
                foreach ($existing_amount_detail as $detail) {
                    $merged_amount_detail[$index] = $detail;
                    $index++;
                }
    
                // Add new data to the merged array with sequential keys
                foreach ($new_amount_detail as $detail) {
                    $merged_amount_detail[$index] = $detail;
                    $index++;
                }
    
                // Encode back to JSON
                $updated_amount_detail = json_encode($merged_amount_detail);
    
                // Update the database
                $this->db->set('amount_detail', $updated_amount_detail);
                $this->db->where('student_fees_master_id', $data['student_fees_master_id']);
                $this->db->where('fee_groups_feetype_id', $data['fee_groups_feetype_id']);
                return $this->db->update('student_fees_deposite');
            } else {
    ...
    public function getFeeCollectionReport($start_date, $end_date, $feetype_id = null, $received_by = null, $group = null, $class_id = null, $section_id = null,$fee_groups_id=null)
    {
        ...
        if ($fee_groups_id != null) {
            $this->db->where('fee_groups_feetype.fee_groups_id', $fee_groups_id);
        }

...
class Student_model extends MY_Model
{
    ...
    public function getLocationStudents()
    {
        $this->db->select('
            ...
            students.rte,
            students.gender,
            IFNULL(students.location_id, 0) as `location_id`,
            IFNULL(locations.locations, "") as location_name,
        ...
        $this->db->join('categories', 'students.category_id = categories.id', 'left');
        $this->db->join('users', 'users.user_id = students.id', 'left');
        $this->db->join('locations', 'students.location_id = locations.id', 'left'); // Join with the locations table
    ...
    public function locationsearchByClassSectionWithSession($class_id = null, $section_id = null, $location_id = null, $session_id = null)
    {
        $this->db->select('
            classes.id AS `class_id`,
            student_session.id AS student_session_id,
        ...
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('locations', 'students.location_id = locations.id', 'left');
        $this->db->join('categories', 'students.category_id = categories.id', 'left');
        $this->db->where('student_session.session_id', $this->current_session);
        $this->db->where('students.is_active', 'yes');
        
        // Only apply class_id filter if provided
        if (!empty($class_id)) {
            $this->db->where('student_session.class_id', $class_id);
        }
        
        // Only apply section_id filter if provided
        if (!empty($section_id)) {
            $this->db->where('student_session.section_id', $section_id);
        }
        
        // Modify location_id filter to include NULLs if no location_id is provided
        if (!empty($location_id)) {
            $this->db->where('students.location_id', $location_id);
        } 

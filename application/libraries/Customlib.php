...
class Customlib
{
    ...
    public function getBaseUrl()
    {
		...
		if(getenv("APP_ENV") == "production"){
			$base_url  = 'https://'.$subdomain.'.acadiv.in/schooldata/'.$subdomain.'/';
		}else{
			$base_url  = 'http://'.$subdomain.'.localhost/acadiv-school/';
		}
    ...
    public function getBirthDayStudents($date)
    {
        // Select the required fields including class and section details
        $this->CI->db->select('students.id, students.firstname, students.dob, classes.class, sections.section,students.image');
        $this->CI->db->from('students');
    
        // Join with student_session to get class_id and section_id
        $this->CI->db->join('student_session', 'student_session.student_id = students.id');
        
        // Join with classes to get class name
        $this->CI->db->join('classes', 'student_session.class_id = classes.id');
        
        // Join with sections to get section name
        $this->CI->db->join('sections', 'student_session.section_id = sections.id');
    
        // Filter students who have a birthday on the specified date
        $this->CI->db->where("DATE_FORMAT(students.dob, '%m-%d') = DATE_FORMAT('" . $date . "', '%m-%d')");
        
        // Order the results by student ID
        $this->CI->db->order_by('students.id');
...
{
    $this->CI->db->select("staff.*, staff_designation.designation, department.department_name as department, roles.id as role_id, roles.name as role");
    ...
    $this->CI->db->join('department', "department.id = staff.department", "left");
    $this->CI->db->where('staff.is_active', $is_active);
    $this->CI->db->where("DATE_FORMAT(staff.dob, '%m-%d') = DATE_FORMAT('" . $dob . "', '%m-%d')");
    
   
    $this->CI->db->order_by('staff.id');
    $query = $this->CI->db->get();
